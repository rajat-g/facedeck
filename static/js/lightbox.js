...
import { $, api, basename, escapeHtml, faceUrl, facelessUrl, shortDir, sourceUrl, state } from "./core.js";
...
export function openFacelessViewer(items, index, total) {
    state.lightbox = {
        kind: "faceless",
        groupId: null,
        groupName: "No faces detected",
        items: [...items],
        index: Math.max(0, index),
        total: total ?? items.length,
    };
...
function renderViewer() {
    const lb = state.lightbox;
    if (!lb) return;
    // Drop any tag overlay from the previously shown photo: tag boxes are
    // painted asynchronously, so without this a faceless photo (or face crop)
    // opened after a tagged one would keep showing the old boxes.
    ...
    img.src = isFace ? faceUrl(lb.groupId, current)
        : lb.kind === "faceless" ? facelessUrl(current)
        : sourceUrl(lb.groupId, current);
    img.alt = name;
    img.title = current;
    ...
    meta.innerHTML =
        ...
        `<div class="meta-path" title="${escapeHtml(current)}">${escapeHtml(isFace ? `Face crop · group ${lb.groupId}` : shortDir(current, 60))}</div></div>`;
    ...
    } else if (lb.kind === "faceless") {
        const note = document.createElement("div");
        note.className = "muted small";
        note.textContent = "No faces were detected in this photo — nothing to group. It stays listed here until a re-run finds faces.";
